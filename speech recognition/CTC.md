...
#### 수식 표현 및 주요 정의
   - **입력 시퀀스** $ x $: 길이 $ T $의 시퀀스이며, 각 요소는 $ m $-차원의 실수 벡터.
   - **네트워크 출력**: $ y = N_w(x) $으로 정의되며, 각 시간 $ t $에서의 활성화 값 $ y^t_k $는 라벨 $ k $가 나타날 확률을 의미함.
   - 확률 분포 정의:
     $\[
     p(\pi|x) = \prod_{t=1}^{T} y^t_{\pi_t}, \quad \forall \pi \in L_0^T
     \]$
   - **경로 함수 \( B \)**: 경로 \( \pi \)에서 빈 라벨과 중복된 라벨을 제거하여 실제 라벨 시퀀스를 생성.
   - 최종적으로, 특정 라벨링 $ l $에 대한 조건부 확률은 그 라벨링에 해당하는 모든 경로의 확률을 합산하여 구함:
     $\[
...
### 🔎 입력 시퀀스의 길이가 Target 시퀀스의 길이보다 긴 이유
- 타겟 시퀀스는 "말하고자 하는 의미"의 단위이기 때문에 짧고 간결하게 표현되는 반면, 입력 시퀀스는 해당 라벨을 표현하기 위한 신호의 전체 흐름을 포함해야 하므로 길어질 수 있습니다
- 예시
  - 음성 인식에서 "HELLO"라는 단어를 발음한다고 가정
  - 입력 시퀀스: 음성을 아주 짧은 시간 간격으로 샘플링한 데이터로, 예를 들어 1초를 100개로 나누어 각 시간마다의 파형을 수집한다고 하면, "HELLO"라는 단어가 발음되는 동안에 100개 이상의 입력 데이터가 생성
  - 타겟 시퀀스: 타겟 시퀀스는 단어 "HELLO"에 대응하며, 각 음소는 "H", "E", "L", "L", "O"처럼 표현 

### 🔎 입력 공간이 𝑚 차원의 실수 벡터 시퀀스로 구성되는 이유
- 다양한 특성을 가진 데이터를 시간적 순서에 따라 효과적으로 표현하기 위해서
- m 차원 벡터는 여러 피처를 동시에 다룰 수 있는 수학적 구조를 제공해주기 때문에 다양한 문제에 대한 유연한 해결이 가능
- **다수의 피처(feature)** 를 포함하는 벡터를 사용하면, 모델이 데이터를 더 정교하게 학습할 수 있음
- 예를 들어, 음성 데이터는 특정 순간의 주파수, 음량 등 다양한 정보를 포함할 수 있으며, 이러한 특성들이 𝑚 차원의 벡터에 담김
