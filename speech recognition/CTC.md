...
- 하지만 한계점이 존재
  ...
  - 데이터의 독립성 가정을 명시적으로 설정해야함
  - HMM은 generative model이기에, sequence labeling에서도 discriminative model적으로 접근하지 못함
- 이와 다르게, RNN은 인풋, 아웃풋 외에 데이터에 대한 사전 지식이 필요하지 않으며, 내부 상태를 통해 데이터를 순차적으로 처리하며, 시간적/공간적 노이즈에 강한 모델
- 하지만 R과거에는 시퀀스 레이블링에 직접적으로 사용되지 못함
  - 표준 신경망 목적 함수가 시퀀스의 각 지점에 독립적으로 정의되었기 때문
  - 따라서 학습 데이터가 사전 분할되어야 하고, 최종 레이블 시퀀스를 생성하기 위해 네트워크 출력을 후처리해야함
- 최근까지, sequence labeling에 쓰이는 RNN의 가장 효과적인 사용은 HMM - RNN 하이브리드 모델
  - 하지만 앞서 언급된 HMM의 한계점으로, 본 모델은 RNN의 장점을 다 활용하지 못함 
- 고로 본 논문에서는 **CTC(연결 시간 분류)** 라는 새로운 방법을 제시하여, RNN을 사전 분할이나 후처리 없이 시퀀스 데이터를 라벨링할 수 있는 모델로 사용할 수 있음을 설명
...
- **U ≤ T**
  - 입력 시퀀스와 Target 시퀀스의 **길이가 일반적으로 동일하지 않기에 미리 정렬할 방법 없음**
...
> 이렇게 하면 초기 규칙은 다음과 같이 주어집니다:
> $\[\alpha_1(1) = y_b^1\]$
> $\[\alpha_1(2) = y_{l_1}^1\]$
> $\[\\alpha_1(s) = 0, \quad \forall s > 2\]$
> 그리고 재귀 관계식은 다음과 같습니다:
> **(6)**   **$\[\alpha_t(s) = \begin{cases} \bar{\alpha}_t(s) y_{l_s'}^t & \text{if } l_s' = b \text{ or } l_{s-2}' = l_s' \\(\bar{\alpha}_t(s) + \alpha_{t-1}(s-2)) y_{l_s'}^t & \text{otherwise} \end{cases} \tag{6}\]$**
> 여기서 :
> **(7)**  **$\[\bar{\alpha}_t(s) = \alpha_{t-1}(s) + \alpha_{t-1}(s-1) \tag{7}\]$**
