...
- **개요**
  - **프레임 동기식(시간 기준)** 으로 동작하는 빔 서치 알고리즘
  - **입력의 각 프레임 t 단위**로 빔 탐색 및 가지치기를 수행
  - 특히 **CTC, RNN-T, RNA와 같은 명시적 정렬 기반 모델에 적합**
- **작동 방식**
  - 각 시간 프레임 t에서, C₁:ᵢ(t)는 지금까지 입력된 프레임까지 예측된 레이블 시퀀스를 의미
  - 아래 수식처럼 프레임 t에서의 **확장된 부분 가설 집합**을 기반으로 **빔 가지치기 수행:**
     - C̃₁:ᵢ(t) = NBEST_{C₁:ᵢ(t)} P(C₁:ᵢ(t) | X),   where   |C̃₁:ᵢ(t)| = Δ
  - 여기서 확률 P(C₁:ᵢ(t) | X) 는 가능한 모든 정렬 A₁:ₜ ∈ 𝒜(X, C₁:ᵢ(t)) 에 대해 합산되어 계산됨:
     - P(C₁:ᵢ(t) | X) = ∑_{A₁:ₜ ∈ 𝒜(X, C₁:ᵢ(t))} P(A₁:ₜ | X)
- **Label Synchronous 방식과 비교**
  - Label Synchronous
     - **출력 토큰 단위(label 단위)** 로 빔 탐색을 진행하며, 정렬 필요없음(암시적)
     - 유연하고 적용 범위 넓음
     - 정렬 정보가 없어 긴/짧은 예측 불균형 문제 존재
  - Frame Synchronous
     - **입력 프레임 단위** 로 빔 탐색을 진행하며, 정렬 필요(명시적)
     - 온라익/스트리밍 적용 쉬움
     - 정렬 처리 필요로 복잡도 있음
- **특징 및 장점**
  - **명시적 정렬 정보**를 포함하므로 각 입력 프레임에 대해 어떤 레이블을 출력할지 명확하게 추적 가능
  - **스트리밍 처리에 적합** → 실시간 ASR 시스템에서 유리
  - **기존 WFST 기반 시스템**이나 **HMM 기반 디코더**와 유사한 방식으로 구현 가능
  - 다양한 **언어 모델 통합 가능** (N-gram LM, neural LM 등)

- **활용 예시**
  - CTC Prefix Beam Search
     - 모든 가능한 정렬을 주변화하여 각 프레임에서 레이블 시퀀스 확률 계산
     - 빠르고 정확한 디코딩 가능
  - RNN-T 디코더
     - 프레임 간 블랭크와 레이블 사이의 전환 상태에서 **언어 모델 점수와 통합 가능**
  - Triggered Attention 기반 AED + LM 결합 디코더
     - CTC에서 생성된 가설에 대해 attention 기반 AED로 실시간 재채점 가능

### E. 블록 단위 디코딩(Block-wise decoding)
- **개요**
  - **Block-wise decoding**은 음성 입력 시퀀스를 고정된 **청크(block) 단위**로 나눠서 처리하는 방식의 디코딩 기법
  - 일반적인 streaming보다 덜 인과적이며, 각 블록 내에서는 **미래 정보(context)** 를 일부 활용할 수 있어 더 정확한 디코딩이 가능
  - CTC나 RNN-T처럼 프레임별 디코딩이 아닌, **청크(블록) 단위로 처리**되는 것이 핵심
- **작동 방식**
  - 입력 음성 특징 시퀀스를 고정된 길이의 블록(청크)로 나눔
  - 각 블록은 다음과 같은 인코더 구조를 사용할 수 있음
     - BLSTM (Bidirectional LSTM)
     - Unidirectional LSTM with output delay (출력 지연을 통해 미래 정보를 일부 활용)
     - Transformer 및 그 변형들
     - 이들은 모두 블록 내에서 **미래 컨텍스트 활용 가능!**
  - 블록 내에서는 완전히 인과적인 구조가 아니기 때문에 **성능 손실을 줄일 수 있음**
  - 단, 블록의 크기가 클수록 정확도는 올라가지만 **대기 시간(latency)** 도 함께 늘어나므로 trade-off 조절이 중요
- **왜 중요한가?**
  - **기존의 fully causal 구조는 미래 정보 활용 불가** → 정확도 한계 있음
  - 반면 block-wise decoding은 **미래 컨텍스트 활용 가능** → 더 정확한 예측 가능
  - 특히 **AED 모델**에선 효과적
     - 입력 특징 ↔ 출력 레이블 사이의 **블록 단위 정렬 제약(monotonicity)** 를 부여할 수 있어 구조적으로 자연스러움
     - 이 덕분에 **AED 모델로도 스트리밍 디코딩 구현 가능!**

### F. 디코딩 중 모델 융합(Model fusion during decoding)
- **개요**
  - **디코딩 과정에서 여러 모델의 점수를 조합(fusion)** 하여 인식 성능을 향상시키는 전략
  - 과거 **HMM 기반 beam search**에서 다양한 점수를 조합했던 것과 유사
  - 대표적으로 **E2E 모델의 출력 확률** + **외부 언어 모델(LM) 점수**를 결합
- **동기식(Synchronous) 점수 융합**
  - 시간 단위가 **일치**하는 경우 (예: 프레임 단위 혹은 레이블 단위)에는 **동기식(synchronous)** 으로 점수를 쉽게 합칠 수 있음
  - **대표 예시: LM Shallow Fusion**
     - **외부 언어 모델(LM)** 의 확률을 beam search 중 E2E 모델 점수에 가중합 형태로 추가
     - log P(C₁:ᵢ | X) ← log P(C₁:ᵢ | X) + γ · log P_lm(C₁:ᵢ)
     - γ: 언어 모델의 가중치
- **비동기식(Asynchronous) 점수 융합**
  - **서로 시간 단위가 다른 모델**의 점수 결합을 의미
     - 예: CTC(RNN-T) → 프레임 단위, 어텐션/LM → 레이블 단위
  - 시간 불일치를 조정하기 위한 추가적인 처리 필요
  - 프레임 ↔ 레이블 시간 매핑 방식
     - **Time-synchronous beam search**
        - **특정 레이블 전환 상태**에서 LM 점수를 삽입
        - 예: CTC에서 공백(blank) 기호 뒤에 단어 경계에서 LM 점수 추가
     - **Label-synchronous beam search**
        - 주어진 레이블 시퀀스에 대해 **가능한 모든 정렬을 주변화(marginalization)**
        - 예: CTC + Attention 결합 디코딩
- **고려 사항**
  - **동기식 융합은 단순하고 빠름**
     - 단일 패스 디코딩(one-pass decoding)이 가능
  - **비동기식 융합은 성능 향상 가능성↑**
     - 프레임-레이블 시간 단위 매핑을 위한 **동적 프로그래밍(dynamic programming) 필요**
     - 특히 **Label-synchronous beam search**에선 계산량이 큼 → **발화 길이에 따라 비용 커짐**
     - 이를 줄이기 위한 **휴리스틱(heuristic)** 도 함께 활용됨

### G. 점수 융합 중 어휘 제약(Lexical constraint during score fusion)
- **개요**
  - 전통적인 ASR 시스템은 **단어 단위 언어 모델을** 활용하여 **문맥 정보와 어휘 제약을 함께 반영**
  - 하지만 대부분의 **E2E ASR은 문자/토큰 단위로 동작** → 이로 인해 **단위 불일치(mismatch)** 문제가 발생
  - 이를 해결하기 위한 **단어 기반 제약을 E2E ASR에 통합하는 다양한 방법**이 제안
- **WFST 기반 전통 방식 (TLG 그래프)**
  - WFST(Weighted Finite-State Transducer) 기반 전통 접근법
     - T (token-to-phoneme), L (lexicon), G (language model)를 결합한 **TLG graph**생성
     - 이 **TLG를 통해 E2E 모델 출력(토큰 기반)** 과 **단어 기반 제약**을 연결
  - **CTC, attention 모델 모두 적용 가능**
     - CTC의 경우: 공백(blank) 토큰 처리를 고려한 그래프 설계 필요
     - Attention 기반 모델에도 적용 가능
- **단어 경계 기반 동기화 방식**
  - **단어 경계**를 나타내는 **공백 기호(blank)** 를 단어 **시작/종료 기준점**으로 활용
  - 이 시점에서 **단어 기반 언어 모델 점수를 삽입**
     - 예: 공백 후 새로운 단어가 시작될 때, 해당 단어의 확률을 LM으로부터 가져와 융합
- **프리픽스 트리(prefix tree) 기반 통합**
  - **문자 기반 모델과 단어 기반 모델을 동기화**하기 위해 prefix tree(접두어 트리) 구조 사용
     - 입력된 문자 시퀀스가 구성 가능한 단어들의 접두어에 해당하는지 여부를 탐색
     - **단어 경계에 도달하면 LM 점수를 추가하거나, 다음 가능한 문자들을 제약**
  - **확장 사례:**
     - **sub-word 단위 (예: BPE, SentencePiece)** 에도 사용 가능
     - 토큰 기반에서 **word-level 언어 모델을 lookahead 구조**로 활용
     - 예: 다음 단어의 확률을 미리 계산하여 문자 디코딩 시점에 반영

### H. 다중 패스 융합(Multi-pass fusion)
- **핵심 개념**
  - 앞서 설명된 대부분의 융합 기법은 **beam search 과정 중 동기화된 점수 융합** 방식 (예: shallow fusion) → **one-pass decoding**
  - 하지만 **multi-pass fusion**은 **n-best 결과나 lattice(격자)** 를 활용하여 **후처리 단계에서 더 깊이 있게 가설을 재평가**하는 방식
  - 일명 **후처리 재점수(Rescoring)** 또는 **2차 디코딩**

- **Multi-pass 방식의 예시들**
  - N-best rescoring
     - E2E 모델의 출력을 단어 기반 LM으로 재점수
  - Forward + Backward 디코딩 결합
     - BiLSTM 디코더를 활용해 전체 문맥 고려
  - Two-pass 구조(ex. CTC -> Attention)
     - 1차: 실시간 응답, 2차: 성능개선
- **Multi-pass의 구조 및 장점**
  - **1차 패스**: 빠르게 스트리밍 응답 제공 (CTC, RNN-T 기반)
  - **2차 패스**: Attention 모델을 사용해 **출력 후보(N-best)** 또는 **lattice** 기반으로 정밀한 재점수 → **최종 결과 개선**
  - 실시간성과 정확도 동시 만족
  - 문맥을 더 깊게 반영하여 오류 보정 가능
 
- **Lattice 기반 Multi-pass의 필요성**
  - **Lattice(격자)** 는 N-best보다 풍부한 후보 정보를 담고 있어:
     - **spoken term detection, SLU, word posterior** 계산 등에도 활용 가능
  - 그러나 문제 발생:
     - **RNN-T와 AED** 모델은 **Markov 가정이 없어서** 가설을 쉽게 병합하지 못함 → lattice 생성이 어려움
  - 해결방안
     - 출력 이력을 고정 길이로 제한 - RNN-T의 출력을 제한하여 Markov 가정 유사하게 만듦
     - Beam search 중 유사 가설 병합 - 동일한 RNN-T 구조 유지하면서도 lattice 유사한 구조 생성 가능

### I. 가설 및 발화 단위 벡터화(Vectorization across both hypotheses and utterances)
- **개념**
  - **기존의 beam search 디코딩**은 각 가설(hypothesis)에 대해 **하나하나 순차적으로 계산**
  - 하지만 **벡터화(Vectorization)** 를 적용하면:
     - 여러 가설들을 하나의 **벡터 또는 행렬**로 처리할 수 있음
     - **벡터-행렬 연산(vector-matrix operations)** 을 통해 효율적으로 점수 누적 가능
- **장점**
  - 가속화 효과로 여러 가설을 한 번에 처리 → CPU, GPU 등의 **병렬 연산 자원**을 더욱 효율적으로 활용
  - 속도 대폭 향상 가능
  - 배치 처리 가능으로 여러 **발화(utterance)** 도 동시에 처리 가능 → 실시간 또는 대규모 처리에 유리
- **적용 사례 및 기술**
  - **적용 모델**
     - RNN-T 기반 디코더
     - Attention 기반 디코더
  - 적용 툴킷
     - TensorFlow+Fairseq : 일반적인 Seq2Seq 모델용 벡터화된 beam search 인터페이스 제공
     - ESPnet : E2E ASR용 주요 오픈소스, 벡터화 빔서치 지원
     - ESPRESSO : Fairseq 기반 음성 인식 툴킷, 멀티 유터런스 처리 지원
     - RETURNN : 고성능 E2E 학습 및 디코딩 프레임워크



## 6. 언어 모델 통합
- **언어 모델의 역할**
  - **ASR의 문장 선택**을 더 잘하도록 도와주는 **확률 모델**
  - 목표: 음향 정보만으로는 부족한 **문장 자연스러움**을 반영
  - 확률적 모델로, 전체 문장 C에 대한 확률 P(C)를 추정
     - 전통적인 N-gram 기반 LM
     - 현대적인 Neural LM (예: RNN, Transformer 등)
- **Hybrid ASR vs. End-to-End ASR의 LM 통합 방식**
  - Hybrid ASR
     - 별도 LM 사용 (ex. WFST + N-gram)
  - E2E ASR
     - 디코더가 내재적으로 LM 역할 수행
     - TNN-T의 prediction network
     - AED의 decoder 등
  - 하지만 **E2E ASR도 별도 LM 통합이 여전히 유의미한 개선**을 보여줌
 
- **E2E 모델이 외부 LM을 여전히 사용하는 이유**
  - 일반화 보완
     - E2E 모델은 단일 구조로 복잡한 매핑 학습 → 훈련 데이터가 부족하면 과적합 위험
     - Pretrained LM은 **덜 일반화된 예측을 보정**하는 데 도움
  - 외부 텍스트 데이터 활용
     - LM은 **음성이 없이 텍스트만으로도 훈련 가능**
     - 텍스트는 대량으로 확보하기 쉽고, **학습 속도도 빠름**
     - E2E 모델의 부족한 언어 정보 커버 가능
  - 도메인 적응(Domain Adaptation)
     - E2E 모델은 **도메인 전환이 어려움** - 새로운 도메인 → 쌍을 이룬 음성-텍스트 필요
     - LM은 도메인 특화 텍스트만 있으면 적응 가능 - **모델 재학습 없이** 도메인 특화된 보정 가능

### A. 언어 모델(Language models)
- **목적**
  - E2E ASR에서 **언어 모델(Language Model, LM)** 은 텍스트 시퀀스에 대한 사전 확률 P(C)을 제공
  - LM은 각 토큰이 이전 토큰 시퀀스에 대해 조건부로 발생할 확률 P(cᵢ | c₀, c₁, …, cᵢ₋₁) 로 정의
  - 이를 체인 규칙(chain rule)으로 표현: P(C) = P(c₁ | c₀) × P(c₂ | c₀, c₁) × … × P(cₗ₊₁ | c₀, …, cₗ)
     - c₀ = ⟨sos⟩, cₗ₊₁ = ⟨eos⟩

- **N-gram LM**
  - **Markov 가정 기반**: 다음 토큰은 직전 N-1개의 토큰에만 의존
  - 확률: P(cᵢ | cᵢ₋ₙ₊₁ … cᵢ₋₁) ≈ K(cᵢ₋ₙ₊₁, …, cᵢ) / ∑ K(cᵢ₋ₙ₊₁, …, cᵢ)
     - 여기서 K는 빈도 수
  - 장점: 간단하고 빠르며 WFST 등으로 쉽게 통합 가능
  - 단점: 희소성 문제 → 스무딩 필요, 긴 컨텍스트 반영 불가

- **FNN-LM (Feed-forward Neural Network LM)**
  - N-1개 입력 토큰 → 임베딩 → MLP → Softmax로 다음 토큰 예측
  - 수식 구조 예시
     - P(cᵢ | cᵢ₋ₙ₊₁:ᵢ₋₁) = Softmax(Wₒ · hᵢ + bₒ)
     - hᵢ = tanh(Wₕ · eᵢ + bₕ)
     - eᵢ = concat(E(cᵢ₋ₙ₊₁), ..., E(cᵢ₋₁))
  - 장점
     - **분산 표현(embedding)** 을 활용 → 더 나은 일반화
     - N-gram보다 부드러운 확률 추정 가능
- **RNN-LM**
  - 재귀 구조로 이전 숨겨진 상태 hᵢ = RNN(eᵢ, hᵢ₋₁)와 입력 임베딩 eᵢ = E(cᵢ₋₁) 사용: hᵢ = tanh(Wₕ × eᵢ + Wᵣ × hᵢ₋₁ + bₕ)
  - 장점
     - 긴 컨텍스트 반영 가능 (이론상 무제한)
     - LSTM이나 GRU 사용 시 장기 의존성 유지 가능
  - 단점
     - **병렬화 어려움**, 느린 훈련 및 추론
- **ConvLM (Convolutional LM)**
  - 시간 컨볼루션을 활용해 순차 처리 대체
     - hᵢ = h′ᵢ ⊗ σ(gᵢ)
     - h′ᵢ = eᵢ₋ₖ₊₁:ᵢ ∗ W + b
     - gᵢ = eᵢ₋ₖ₊₁:ᵢ ∗ V + c
  - 장점
     - **병렬화 용이**, 빠른 추론 가능
     - 컨텍스트 제한 있지만 RNN-LM에 가까운 성능 제공
     - Residual connection과 stacking으로 깊은 모델 가능
- **Transformer LM**
  - **Self-Attention** 기반 구조:
     - hᵢ = FFN(h′ᵢ) + h′ᵢ
     - h′ᵢ = MHA(eᵢ, e₁:ᵢ, e₁:ᵢ) + eᵢ
  - 장점
     - 긴 시퀀스도 병렬화로 처리 가능
     - 모든 이전 토큰을 한 번에 참조 → 문맥 반영 우수
  - 단점
     - **복잡도: O(N²)** → 긴 시퀀스에는 비효율적
  - 해결법
     - **Transformer-XL**: 과거 hidden state를 재사용
     - **Compressive Transformer**: 오래된 중요한 정보를 압축 저장


### B. E2E ASR에서의 언어 모델 결합 방식(Fusion approaches)
- **개요**
  - E2E ASR의 성능을 향상시키기 위해 **외부 언어 모델(LM)** 을 결합하는 여러 방식이 제안됨
  - 이 단락에서는 대표적인 **4가지 Fusion 방식**을 소개하고 각각의 특징을 정리
- **Shallow Fusion**
  - **가장 일반적이고 구현이 간단**한 방식
  - **Inference 시점**에 E2E 모델과 외부 LM의 점수를 단순히 로그-선형 조합으로 결합함
  - 수식
     - Score(C | X) = log P(C | X) + γ · log P(C)
     - γ: 언어 모델의 점수에 가중치를 주는 스케일링 파라미터
  - 특징
     - 구현 간단, 사전 학습된 LM을 그대로 사용 가능
     - 도메인 차이가 크지 않은 경우 효과적
- **Deep Fusion**
  - **joint network**를 통해 E2E 모델과 LM을 **깊은 수준에서 결합**
  - LM 상태와 E2E 상태를 **게이팅 메커니즘**을 통해 통합
  - 모든 파라미터를 함께 학습하며 두 모델이 **협력적으로 작동**하도록 유도
  - 특징
     - LM과 E2E 모델의 **상호작용이 더 깊음**
     - 학습 시 복잡성 증가, 도메인 불일치 상황엔 약함
- **Cold Fusion**
  - Deep Fusion과 유사하지만 **LM은 학습 중 freeze**되어 있음
  - E2E 모델은 고정된 LM을 **참조만 하도록 학습**
  - 목적
     - LM의 역할을 줄이고, **언어적 bias를 약화**
     - 소스 도메인(학습 데이터)과 대상 도메인(테스트)의 **도메인 간 차이에 강건**
  - 확장형
     - **Component Fusion**: Cold Fusion을 기반으로 LM과 ASR 훈련 데이터를 함께 이용해 데이터의 **bias 완화**에 집중함
- **Internal LM Estimation**
  - 훈련 데이터에서 E2E 모델이 갖는 **내부 LM(Language Prior)** 을 추정하여 **bias를 보정**
  - 기존 shallow fusion의 문제:
     - 모델이 학습 데이터의 언어적 특성에 과도하게 적응할 수 있음 (즉, 소스 도메인 bias)
  - 해결 방법:
     - **내부 LM을 추정**하고, 그것을 빼고 **대상 도메인의 외부 LM**을 더함
  - 수식:
     - Score(C | X) = log P_ϕ(C | X) − γ_ϕ · log P_ϕ(C) + γ_τ · log P_τ(C)
     - ϕ: 소스 도메인
     - τ: 대상 도메인
     - 첫 항: 원래 ASR 점수
     - 둘째 항: 소스 도메인의 언어적 bias 제거
     - 셋째 항: 대상 도메인의 LM 정보 추가
  - 이 접근은 고전적인 **HMM 시스템에서의 음향 모델 점수처럼** 작동
     - P_ϕ(C | X) / P_ϕ(C) ∝ P_ϕ(X | C)
     - 즉, ASR 점수를 **음향 확률처럼 해석** 가능
- **추가 기술: Internal LM 활용 사례**
  - HAT (Hybrid Autoregressive Transducer)
     - RNN-T에서 **인코더 출력을 제거했을 때의 모델**을 Internal LM으로 간주
  - Bias vector 방식 (AED 기반)
     - context vector나 디코더 입력을 기반으로 LM bias를 추정하고, 이를 별도 벡터로 표현하여 LM에 주입
  - Zero-input LM
     - 인코더 출력을 0으로 고정하여 internal LM을 추정 (RNN-T 구조 그대로 활용)


### C. 대규모 사전학습 언어모델을 E2E ASR에 활용하기(Use of large-scale pretrained LMs)
- **개요**
  - 최근 자연어 처리(NLP) 분야에서는 **대규모 텍스트 데이터로 사전 학습된 언어 모델(LM)** 들이 다양한 작업에서 탁월한 성능을 보여주고 있음
  - 대표 모델
     - BERT [244]: Bidirectional Encoder Representations from Transformers
     - GPT-2 [245]: Generative Pre-trained Transformer 2
     - 둘 다 **Transformer** 구조 기반으로 사전 학습됨
- **E2E ASR에서의 활용 방식**
  - E2E 음성 인식 시스템에서도 이러한 **사전학습된 LM들을** 두 가지 주요 방식으로 활용할 수 있음:
  - **N-best Rescoring (후처리 재점수화)**
     - E2E 모델이 beam search를 통해 생성한 **N개의 후보 문장 리스트(N-best list)** 에 대해,
     - 사전학습 LM을 사용하여 각 후보 문장에 **언어적 자연스러움 점수를** 다시 계산함
     - 더 **정확하고 문맥적으로 자연스러운 문장**을 최종 결과로 선택 가능
  - **대화 문맥 임베딩 (Contextual Embedding)**
     - 대화형 ASR 환경에서는 **이전 발화나 문맥 정보**가 현재 발화 인식에 중요함
     - 사전학습된 LM (예: BERT)을 사용해 **이전 대화 발화들을 임베딩**하고,
     - 이 임베딩을 E2E 모델에 입력하여 **문맥 인식 능력 향상**
- **장점**
  - **사전 학습된 모델을 바로 활용 가능** → 데이터 부족한 상황에서도 강력한 효과
  - 언어 지식이 풍부하게 내재되어 있어 **자연스러운 문장 생성**에 도움
  - 기존 E2E 모델의 한계를 보완할 수 있는 **외부 지식 소스** 역할 수행




## 7. 전통적인 ASR과 E2E ASR의 관계 정리

### 1) E2E 시스템의 철학과 구조적 차이
- E2E ASR은 전통적인 ASR의 **모듈 분리 구조(AM + LM + lexicon) 와 달리**, 하나의 글로벌 joint 모델로 학습과 추론을 모두 수행
- 별도의 lexicon 없이 문자 단위 입력/출력을 통해 동작하며, **단일 전사된 훈련 데이터만으로 학습 가능**
- 충분한 데이터가 주어질 경우 **도메인 지식 의존도가 낮고**, 훈련 프로세스가 단순화됨

### 2) 정렬 방식의 변화
- 전통적인 ASR: **HMM 기반 정렬**, align path marginalization 필요
- E2E
  - **Attention**: 정렬 정보를 soft하게 학습
  - **CTC, RNN-T**: HMM과 유사한 방식으로 정렬 marginalization
  - 특히 RNN-T는 segmental model, direct HMM과 동일한 구조로 해석될 수 있음

### 3) 디코딩 전략: 빔 서치와 단일 패스
- 전통 ASR과 E2E **모두 beam search 사용**
- E2E는 **작은 빔 크기(1~100)** 로도 좋은 성능 → **단일 패스 디코딩 가능**
- 전통 시스템은 **multi-pass (n-best, lattice 기반)** 구조를 가짐 → 복잡하지만 유연성 있음

### 4) 언어 모델링과 결합 전략
- 전통 ASR은 AM과 LM을 분리 학습, 대규모 텍스트로 LM 훈련 가능
- E2E는 모델 내에 **implicit LM (내부 언어 모델)** 을 내포함 → 외부 LM과 결합이 어려움
  - 해결책: **internal LM estimation, shallow fusion, cold/deep fusion** 등
  - 주의점: E2E 모델 점수에는 언어 bias가 내재되어 있으므로 보정 필요

### 5) 발음 lexicon 회피
- 전통 시스템: 발음 lexicon과 음소 세트를 필수로 사용
- E2E: **char/subword** 단위로 직접 모델링, lexicon 없이 동작 가능
  - 단, BPE 등의 subword는 여전히 활용됨
  - 적절한 lexicon 없이도 대규모 학습으로 성능 확보 가능
 
### 6) 학습 방식의 차이
- 전통 ASR
  - ANN/HMM 기반 → **alignment 필요**
  - 음향모델(AM)과 언어모델(LM) 따로 학습
  - Gaussian 초기화, CART 기반 클러스터링 등 복잡한 훈련 프로세스
- E2E
  - **alignment-free 학습 가능**
  - 필요 시 초기 정렬로 pretraining 진행 가능
  - 최근에는 regularization, SpecAugment, TTS 등을 통해 학습 안정성 확보

### 7) 적은 리소스 상황에서의 유연성
- 전통 시스템은 lexicon, 음소지식 등 **보조 지식 사용 가능** → low-resource에 유리할 수 있음
- 그러나 잘못된 lexicon이 오히려 성능 저하를 초래할 수 있음

### 8) 실제 성능 비교 (Librispeech & Switchboard 기준)
- 기존 HMM 시스템: test-clean 2.3%, test-other 4.9%
- E2E 최신 모델: **test-clean 1.8%, test-other 3.7%** 까지 성능 향상
- Switchboard 300h 기준:
  - Hybrid ASR: 6.6%
  - E2E 모델: 최대 5.4%까지 도달
...
## 8. E2E 접근 방식의 벤치마크 성능 추세
### 1) 벤치마크 기반 성능 비교 개요
- **분석 대상 데이터셋**
  - **Librispeech**: 읽기(read speech) 기반, 대규모 공개 코퍼스
  - **Switchboard (SWBD)**: 자발적(conversational) 음성 기반의 전화 대화 데이터
- 이 두 벤치마크는 **음성인식 연구자들의 표준 평가 지표**로 널리 사용됨
- 그림 9(SWBD), 그림 10(Librispeech)을 통해 **시간에 따른 E2E 모델의 성능 변화**를 요약

### 2) 성능 개선의 핵심 시점 및 기술
- **2019년 중반 – SpecAugment 도입**
  - **데이터 증강 기술**의 도입이 최초의 큰 전환점
  - SpecAugment
     - **입력 스펙트로그램**에 시간/주파수 마스킹
     - 모델의 일반화 능력 증가
  - **성능에 가시적인 이득**, 특히 작은 데이터셋에서도 강인성 확보
- **이후 – E2E 모델 아키텍처의 발전**
  - **SpecAugment**를 기반으로 한 **아키텍처 개선**이 연달아 이뤄짐
     - **Transformer**: 비인과적 self-attention 기반 구조로 긴 컨텍스트 반영
     - **Conformer**: CNN + Transformer 결합 → 지역 + 전체 정보 동시 처리
     - **ContextNet**: 동적 스케일 context 모듈을 통한 정보 조절
  - 이들 모델은 다양한 데이터 환경에서도 **기존 LSTM 기반보다 더 나은 성능** 달성
- **2021년 이후 – 자기지도/반지도 학습 도입**
  - 추가적인 성능 향상은 **대규모 레이블 없는 음성 데이터 활용 기술**로부터 옴
  - 대표 기술
     - **Self-Supervised Learning (SSL)** – 예: wav2vec 2.0, HuBERT 등
     - **Semi-Supervised Learning (Semi-SL)** – 학습된 모델을 통한 Pseudo-label 사용
  - 특히 **Libri-light (60,000시간)** 등 **도메인 일치 unlabeled 데이터**를 통해 E2E 모델이 더 강력해짐

### 3) 성능 추세 요약
- **2017~2018**: 초기 E2E 모델은 전통적인 ASR 대비 성능 열세
- **2019**: SpecAugment 도입으로 E2E 모델이 전통 시스템과 유사하거나 우수한 성능 달성
- **2020~2021**:
  - Conformer 등 신경망 구조 최적화
  - SSL/Semi-SL 등 비지도 학습 기반 학습 전략 도입
- 현재는 E2E 시스템이 많은 벤치마크에서 **기존 하이브리드 시스템을 능가**

### 4) 결론 및 시사점
- E2E ASR은 **모델 구조 개선 → 학습 전략 발전**의 흐름으로 지속적인 성능 개선을 이루고 있음
- 단순히 end-to-end라는 구조적 단순화에 그치지 않고,
  - **데이터 증강, 모듈 구조 고도화, 비지도 학습 통합** 등을 통해 실제 **음성인식 품질** 향상 달성
- 앞으로도 **라벨 없는 음성 데이터 활용 및 사전학습 기반의 도메인 일반화** 기술이 주도할 것으로 전망


## 9. E2E 모델의 실전 배포
- 이론적 성능만큼이나 중요한 것은 **실제 디바이스나 환경에 배포 가능한가 여부**
- 특히 모바일 환경에서는 **속도, 지연 시간(latency), 메모리 효율, 실시간성 등이 핵심 고려 요소**

### 1) 대표 사례: Google Pixel 스마트폰 시리즈
- **(1) 2019년 – Pixel 4에 최초의 스트리밍 E2E ASR 도입**
  - 세계 최초로 **실시간 스트리밍 E2E 모델이 스마트폰에 배포됨**
  - **모델 구성**
     - **First pass**: 스트리밍 **RNN-T 기반 디코더**
     - **Second pass**: **Attention-based AED** 모델로 재점수 (Rescoring)
  - **컨텍스트 처리**
     - FST 기반 **contextual biasing** 적용 → 고유명사 및 개인화된 쿼리 인식 강화
  - **효율성**
     - **CPU 단독 실행 가능**
     - **실시간보다 빠른 처리 속도** 확보
   
       
- **(2) 2020년 – Pixel 5: 사용자 인지 지연 시간 개선**
  - **지연 시간(latency) 줄이기** 위한 두 가지 핵심 기술 도입:
     - **End-to-end endpointing**: 사용자가 말을 마치면 즉시 인식 종료
     - **FastEmit**: 모델이 **더 빠르게 토큰을 출력**하도록 유도하는 훈련 전략
  - 결과적으로, **사용자가 말한 내용이 화면에 나타나는 시간 간격이 단축됨**

- **(3) 2021년 – Pixel 6: 성능 및 정확도 정점 달성**
  - 하드웨어: **TPU 가속기 활용**
  - 주요 기술 업그레이드:
     - **Conformer** 인코더: 지역/전역 정보를 동시에 반영하는 고성능 인코더
     - **Compact prediction network**: 디코더의 임베딩 및 연산 최소화
     - **2-pass cascaded encoder**: 1차는 스트리밍, 2차는 정확도 보완
     - **Neural LM re-scorer**: 고유 명사(long-tail entities) 인식 정확도 향상
  - 결과:
     - 품질 측면에서 **Google이 출시한 최고 성능 ASR 시스템**
     - **클라우드의 80GB 대형 모델보다 뛰어난 성능**, 모바일 최적화 모델이 성능에서도 우세
   
### 2) 핵심 시사점
- 단순히 좋은 WER이 아닌, **실제 배포 환경에 맞춘 최적화된 설계**가 중요
- Google은:
  - **하드웨어(TPU) + 아키텍처(Conformer) + 트릭(FastEmit, endpointing)** 을 통합해 실시간 환경에 최적화
- 이 사례는 E2E ASR이 **모바일 디바이스에서도 실시간으로 고성능 동작이 가능함을 증명**



## 10. 향후 연구 과제 (Areas for Future Work)
- 현재 **E2E ASR은 연구적으로 매우 활발하지만**, 실제 산업 배포에서는 아직 **고전적인 ASR 방식이 널리 사용됨**
- 이유는 단순히 성능 때문만이 아니라, **도메인 적응, 훈련 효율성, 희귀 이벤트 처리 등 여러 실용적인 과제**가 존재하기 때문

### 1) 주요 한계점 및 연구 과제
- **(1) Low-resource 환경에서의 성능 저하**
  - E2E 모델은 데이터가 풍부할 때는 매우 강력하지만, **적은 데이터에서는 일반화가 부족**
  - 고전적인 방식은 언어모델(LM)을 분리해서 교체 가능하지만, E2E는 **통합 구조로 인해 도메인 적응이 어려움**
  - **연구 필요**
     - Few-shot / semi-supervised 학습
     - 외부 LM 활용을 위한 개선된 fusion 및 internal LM 추정 전략
- **(2) 훈련 효율성**
  - E2E 모델은 **고전적인 모델에 비해 몇 배 더 많은 에포크**를 요구
  - 효과적인 **훈련 스케줄링, curriculum learning, 최적화 전략** 등에 대한 연구 필요
- **(3) 모듈성 손실 및 설명 가능성 저하**
  - 통합 구조는 장점이자 단점
     - 장점: 단일 모델 훈련으로 종단 간 학습 가능
     - 단점: 각 구성 요소(예: AM/LM)에 대한 **분리된 튜닝/이해/분석이 어려움**
  - 특히 **희귀 단어, 화자 적응, 환경 노이즈** 등 세부 요소 제어가 어려움

- **(4) 텍스트 전용 데이터 활용의 어려움**
  - 고전적인 모델은 텍스트만으로도 LM을 학습 가능하지만, E2E는 그렇지 않음
  - 따라서 최근엔
     - **TTS를 활용한 오디오 생성** (보조 방법)
     - **음성-텍스트의 공유 임베딩 공간 구축** 시도 (예: cross-modal learning)
     - **joint training을 통한 modality fusion** 연구 활발
- **(5) AED의 길이 편향 문제**
  - Attention 기반 디코더는 **짧은 시퀀스를 선호하는 경향 (Length Bias) 발생**
  - 현재까지는 **휴리스틱적 해결책**(예: 길이 보정 항, 커버리지 등)이 일반적이나,
  - **이론적으로 정당화된 해결책**은 여전히 부족
- **(6) 강건성(Robustness) & 적응(Adaptation)**
  - **화자 적응, 환경 변화(잡음, 채널 변화 등)에 약한** 경향
  - 특히 불일치 조건(out-of-domain)에서는 성능 급락
  - 연구 필요
     - Meta-learning, domain-invariant representation, unsupervised adaptation 등

### 2) 확장 가능성: 멀티채널 & 멀티태스크
- E2E는 원칙적으로 여러 작업을 동시에 수행 가능
  - 예: **화자 분리 + 분할 + 인식 (multi-speaker ASR)**
- 최근 연구는 **E2E 프레임워크로 이러한 복합 문제를 통합**하려는 시도 진행 중


### 3) 핵심 질문: E2E는 정말 ASR의 미래인가?
- **공통된 최첨단 E2E 구조**가 나올 수 있을까?
- 음향-언어 분리 vs 통합 모델, 과연 어느 쪽이 더 효과적인가?
- 훈련 리소스, 도메인 유연성, 적응성, 효율성 모두 고려할 때
  - 앞으로도 **고전적인 방식과의 혼합 혹은 적절한 병행이 필요한 분야**가 존재
 

## 11. 결론
### 1) 종단 간 ASR의 개념 정리
- 이 논문은 최근 각광받고 있는 **End-to-End (E2E) ASR 접근 방식**을 체계적으로 정리함
- 핵심 아이디어:
  - **음성 입력 → 문자/서브워드 기반 출력**으로 직접 매핑하는 **고도로 통합된 신경망 기반 모델**을 사용
  - 즉, 기존의 **모듈식 ASR 구조** (음향 모델 + 발음 사전 + 언어 모델)를 하나의 모델로 통합하여 학습
 
### 2) 기존 방식 대비 E2E의 강점
- **단일 모델 구조**로 학습이 간단해지고, 전체 파이프라인의 엔드-투-엔드 최적화가 가능
- 특히 최근에는 **on-device 시스템**에 적합한 소형 모델로 발전 (예: Pixel 스마트폰 탑재 사례, 섹션 IX 참고)
- **레이블된 음성-텍스트 페어**만 있으면 학습 가능하다는 점에서 **실용성과 접근성**이 높음
...
- 통합 구조의 단점
  ...
  - 훈련 데이터와 연산량 요구 높음
  - 내재된 **모듈성 및 설명 가능성 부족**
- 이러한 한계는 **섹션 X에서 자세히 논의**되었으며, 현재 활발히 연구 중
