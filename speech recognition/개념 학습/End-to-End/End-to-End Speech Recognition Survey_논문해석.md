...
- 우선 지금 시점에서는, 특정 훈련 예제 (X, C) 가 주어졌을 때, 각 모델은 유효한 정렬(valid alignments) 의 집합 $`{A}_{(T,C)}`$ 를 정의하며, 다음과 같이 모든 가능한 정렬 시퀀스에 대해 주변화(marginalize) 함으로써 정의한다는 점만 이해하면 충분합니다: $$P(C \mid X) = P(C \mid H(X)) = \sum_{A} P(C \mid A, H(X)) \cdot P(A \mid H(X))\\ \quad = \sum_{A \in \mathcal{A}_{(T = |H(X)|, C)}} P(A \mid H(X)) \tag{1}$$
- 여기서, 정의에 따라 $`P(C | A, H(X)) = 1 \quad \text{if} \quad A \in \mathcal{A}_{(T, C)}`$
...
#### 1) CTC
- Connectionist Temporal Classification (CTC) 는 Graves 외 [12]가 제안한 것으로, 입력 토큰 시퀀스를 해당되는 출력 토큰 시퀀스로 매핑하는 기술이다.
- CTC는 인코더 출력 H(X)와 라벨 시퀀스 C 사이의 정렬을 명시적으로 모델링하기 위해 ⟨b⟩로 표기되는 특별한 “blank” 라벨을 도입
- 이로써 확장된 출력 라벨 집합은 다음과 같이 정의된다: $$C_b = C \cup \{\langle b \rangle\}$$
- 정렬 $$A \in C_b^*$$ 는 C 또는 ⟨b⟩ 기호로 구성된 라벨 시퀀스로, 전체 길이는 T (인코더 출력 길이)와 동일하다.
- 특정 훈련 예제 (X, C) 가 주어졌을 때, CTC는 모든 유효한 정렬들의 집합을 다음과 같이 정의한다: $$\mathcal{A}_{\text{CTC}}(X, C) = \{ A = (a_1, a_2, …, a_T) \mid a_t \in C_b \}$$
- 단, 다음 조건을 만족해야 한다: 정렬 A는 연속된 동일 라벨을 먼저 병합(collapse)한 후, 모든 ⟨b⟩ 기호를 제거하면 정확히 C가 되어야 함
- 예를 들어, T = 10, C = (s, e, e) 라고 할 때, $$A = (s, \langle b \rangle, \langle b \rangle, e, e, \langle b \rangle, e, e, \langle b \rangle, \langle b \rangle)$$ 는 유효한 정렬 $$A \in \mathcal{A}_{\text{CTC}}(X, C)$$ 에 속한다.
- 이는 Figure 1에서 보여주고 있다.
- 이 예시에서 볼 수 있듯, 출력에서 반복되는 라벨은 중간에 ⟨b⟩를 삽입하여 구분할 수 있다.
- 앞서 설명한 수식 (1)을 따르며, CTC는 정렬에 대한 모든 가능한 CTC 정렬에 대해 주변화(marginalization)하여, 출력 시퀀스 C의 조건부 확률을 다음과 같이 정의한다: $$P_{\text{CTC}}(C | X) = \sum_{A \in \mathcal{A}_{\text{CTC}}(X, C)} P(A | H(X))$$
- 이 확률은 다시 다음과 같이 전개된다: $$= \sum_{A \in \mathcal{A}{\text{CTC}}(X, C)} \prod{t=1}^{T} P(a_t \mid a_{t-1}, …, a_1, H(X))$$
- 하지만 실제 CTC에서는 다음과 같이 더 간단한 독립 가정을 적용한다: $$= \sum_{A \in \mathcal{A}{\text{CTC}}(X, C)} \prod{t=1}^{T} P(a_t \mid h_t) \tag{2}$$
- 특히 수식 (2)에서 알 수 있듯, CTC는 매우 강한 독립 가정(independence assumption) 을 적용하는데, 이는 시간 t에서의 출력이 다른 시점 출력들과 조건부 독립이며, 오직 해당 시점의 인코더 출력 $h_t$ 만을 기반으로 결정된다는 것이다.
- CTC 모델은 기본적으로, 각 시점 t에서 $P(a_t | X)$ 를 모델링하는 신경망 구조로 구성(그림 2 참고)
- 예를 들어, 이 구조는 인코더(encoder)로 불리는 신경망으로 구현되며, 일반적으로는 다음과 같은 모델이 사용된다: 딥 컨볼루션 네트워크 (CNN), 딥 순환 신경망(RNN), 예: LSTM (Long Short-Term Memory) 셀을 포함한 네트워크
- 이 인코더는 입력 음향 프레임 X 를 인코딩 표현 $$H = (h_1, …, h_T)$$ 로 변환하며, 각 $$h_t \in \mathbb{R}^k$$ (k는 벡터 차원 수)
- 이 인코더는 소프트맥스 레이어와 연결되며, 출력 레이어의 각 노드는 $C_b$에 해당하는 라벨 수만큼 존재
