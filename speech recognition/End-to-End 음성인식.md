...
- Forward Algorithm: 주어진 모델에서 관측된 시퀀스가 나올 확률을 계산하며, GMM을 통해 각 상태에서의 방출 확률 고려
- Viterbi Algorithm: 가장 가능성 있는 숨겨진 상태 시퀀스를 찾는 알고리즘으로, GMM을 사용하여 각 상태에서의 관측값에 대한 최대 가능성 평가
- Baum-Welch Algorithm: GMM의 파라미터(평균, 공분산, 혼합 계수)를 포함하여 HMM의 전이 확률과 초기 상태 분포 학습


### 한계
- GMM-HMM 시스템은 고정된 수학적 가정을 사용해 특정 주파수 패턴을 기반으로 분석하기 때문에,
- 실제로 다양한 음성 신호를 정확하게 처리하기에는 한계가 있었습니다. 특히 노이즈나 발음의 변동성에 취약했습니다.
- GMM-HMM 모델은 많은 파라미터를 포함하고 있어, 학습이 느리고 계산 비용이 높음
- GMM-HMM은 HMM의 한계를 일부 극복했지만 여전히 고차원 데이터의 복잡성을 완벽하게 설명하지 못함
- GMM의 가정(각 상태에서 관찰값이 가우시안 분포를 따른다는 가정)이 모든 음성 데이터에 적합하지 않을 수 있음
